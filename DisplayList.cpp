...
void CDisplayList::Draw( CDC * dDC )
{
	...
	int i_alpha = 0;	//** AMW number of alpha blends performed
	...
	for( int order=(MAX_LAYERS-1); order>=0; order-- )
	{
		...
		if( layer >= LAY_TOP_COPPER )
		{
			...
			//** AMW
			di.Master_layer_color[0] = C_RGB( m_rgb[LAY_BACKGND] );
			di.Master_layer_color[1] = C_RGB( m_rgb[layer] );
		}
		else
		{
...
//		                     m_org_x, m_org_y, 0x00220326);
			...
			di.DC_Master->SetBkColor( C_RGB( m_rgb[layer] ));
...
#endif
#ifdef ALPHA
			//** AMW use alpha blending for the copper areas only
			// this requires either a separate DC for the alpha blend, or that the copper
			// areas on all layers be drawn before the traces
			// for testing, just alpha blend everything
...
#ifdef NEVER
			i_alpha++;
			BLENDFUNCTION blend = { AC_SRC_OVER, 0, 255/i_alpha, 0 };
			BOOL bOK = DC_alpha.AlphaBlend( m_org_x, m_org_y, m_max_x-m_org_x, m_max_y-m_org_y,
			                     di.DC_Master,
			                     m_org_x, m_org_y, m_max_x-m_org_x, m_max_y-m_org_y, 
								 blend );
			if( !bOK )
				ASSERT(0);
#endif
#ifdef NEVER 
			DC_alpha.BitBlt(m_org_x, m_org_y, m_max_x-m_org_x, m_max_y-m_org_y,
			                     di.DC_Master,
			                     m_org_x, m_org_y, SRCCOPY);
#endif
