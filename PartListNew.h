...
public:
	...
	// int GetNumParts(){ return m_size; }			// CPT2 use parts.GetNItems()
	...
	cpart2 * Add( CShape * shape, CString * ref_des, CString * package, 
					int x, int y, int side, int angle, int visible, int glued, bool ref_vis );			// CPT2.  Removed uid arg.
	...
	void MarkAllParts( int mark );									// Done in cpp
	// int Remove( cpart * element );								// CPT2.  Use cpart2::Remove().
	void RemoveAllParts();											// Cone in cpp
