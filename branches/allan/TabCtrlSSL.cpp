...
BOOL CTabCtrlSSL::RemoveSSLPage (int nIndex) {
	...
	m_tabs[nIndex].pTabPage->OnDestroyPage (m_nPageIDs[nIndex]);
...
void CTabCtrlSSL::OnInitPage (int nIndex, int nPageID) {
...
void CTabCtrlSSL::OnSelChanging (NMHDR* pNMHDR, LRESULT* pResult) {
	...
	m_tabs[nIndex].pTabPage->OnDeactivatePage (m_nPageIDs[nIndex]);
...
void CTabCtrlSSL::OnSelChange (NMHDR* pNMHDR, LRESULT* pResult) {
	...
	m_tabs[nIndex].pTabPage->OnActivatePage (m_nPageIDs[nIndex]);
...
void CTabCtrlSSL::OnDestroy (void) {
	...
	if (nCount > 0) {
		for (int i=nCount - 1; i>=0; i--) {
			...
			pDialog->OnDestroyPage (m_nPageIDs[i]);
...
BOOL CTabCtrlSSL::OnCommand (WPARAM wParam, LPARAM lParam) {
	CTabCtrl::OnCommand(wParam, lParam);
...
BOOL CTabCtrlSSL::OnCmdMsg (UINT nID, int nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo) {
...
int CTabCtrlSSL::AddPage (LPCTSTR pszTitle, int nPageID, TabDelete tabDelete) {
    ...
    if (NULL == tabDelete.pTabPage) {
		...
		tabDelete.pTabPage->my_TabCtrl  = this;
		tabDelete.pTabPage->m_PageIndex = nIndex;
		...
		if (tabDelete.pTabPage->OnInitPage (nPageID)) {
